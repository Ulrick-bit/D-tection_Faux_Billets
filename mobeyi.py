...
with col1:
    st.image("billets.jpeg", width=65)
...
fichier_csv = "billets_production.csv"
...
#charger le fichier csv
upload_file = st.file_uploader("charger un fichier csv contenant les données")
...
if upload_file:
    ...
    if not all(col in data.columns for col in colonnes_attendues):
        ...
        # histogramme
        prediction_counts = results_df["prediction"].replace({0: "faux billet", 1: "vrai billet"}).values_counts().re
